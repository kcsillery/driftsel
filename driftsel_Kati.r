...
## Kati: add color and plot argumens
ellipsis <- function(mu, Sig, prob.mass, lwd, col, plot){
    ...
    if(plot) lines(ellipse[,1], ellipse[,2], lwd=lwd, col=col)
    return(ellipse)
} 
...
H.test <- function(popefpost, Gpost, THpost, env, silent=T){
    ...
    mobs = mrand = robs = rrand = rep(NA, nmc)
    for( i in 1:nmc ){
        G = Gpost[,,i] # G matrix
        ...
        ## Kati: use the standardized Mantel
        robs[i] = cor(c(D2), c(env)) # observed product moment
        rrand[i] = cor(c(C2), c(env)) # randomized product moment
    }
    ...
    ## Kati: return a list with H and H*
    return(list(obs=mobs, exp=mrand,
                H=length(which(mobs>mrand)) / length(mobs),
                Hs=length(which(robs>rrand)) / length(robs)))
...
## Kati: CDF=T makes return the posterior distribution of S and not just the mean
S.test <- function(popefpost, Gpost, THpost, silent=T, G.off=F, th.off=F, CDF=F, main=NA){
    ...
    if(CDF) return(cdf)
    else return(mean(cdf))
	}

## pop.S.test <- function(popefpost, Gpost, THpost, silent=T){
##     apost = popefpost
##     nmc = dim(apost)[3]
##     npop <- dim(apost)[1]
##     D = matrix(nrow=npop, ncol=nmc)
##     for(i in 1:nmc ){
##         for(p in 1:pop){
## 		a = apost[p,,i]
## 		G = diag(Gpost[,,i])
...
## Kati: make boxplots to visualize divergence at single traits
viz.1trait <- function(fixedpost, popefpost, Gpost, THpost, prob=0.05, popnames, ylab=NULL){
    ## prob sets the significance level for population-wise S test
    ## popnames: a character vector of short population names for plotting
    
    npop <- length(popnames)
    ## ancestral population mean
    A.mean <- mean(res.anim$fixed.ef[1,1,])
    ## get population order according to mean addtitve trait values today
    pop.div.mean <- apply(res.anim$pop.ef[,1,], 1, mean)
    x <- A.mean+pop.div.mean
    names(x) <- popnames
    ## get full distribution of addtitve trait values today
    pop.div <- as.data.frame(apply(res.anim$pop.ef[,1,], 1, function(a) a+res.anim$fixed.ef[1,1,]))
    names(pop.div) <- popnames
    ## get selection gradients
    beta <- apply(res.anim$pop.ef[,1,], 1, function(a) mean(a/res.anim$G[1,1,]))
    names(beta) <- popnames
    beta <- beta[names(sort(x))]
    ## expected additive trait value today if there has been no selection (drift only)
    drift.trait.value.max <- A.mean + sqrt(2*mean(res.anim$G)*diag(apply(res.anim$theta, c(1,2), mean)))
    drift.trait.value.min <- A.mean - sqrt(2*mean(res.anim$G)*diag(apply(res.anim$theta, c(1,2), mean)))
    names(drift.trait.value.max) <- popnames;         names(drift.trait.value.min) <- popnames
    drift.trait.value.max <- drift.trait.value.max[names(sort(x))]
    drift.trait.value.min <- drift.trait.value.min[names(sort(x))]
    
    boxplot(pop.div[, names(sort(x))], ylim=range(c(pop.div, drift.trait.value.max, drift.trait.value.min)),
            ylab=mytraits.nice.name[i], main="", style="quantile", cex=1.2)

    sig1 <- apply(pop.div[, names(sort(x))], 2, quantile, prob) > drift.trait.value.max
    sig2 <- apply(pop.div[, names(sort(x))], 2, quantile, 1-prob) < drift.trait.value.min
    if(sum(sig1)!=0) mtext("*", 3, -1, at=(1:length(x))[sig1], cex=3)
    if(sum(sig2)!=0)  mtext("*", 3, -1, at=(1:length(x))[sig2], cex=3)
    
    abline(h=A.mean, lwd=2)
    
    polygon(c(1:npop, rev(1:npop)), c(drift.trait.value.max, rev(drift.trait.value.min)), lwd=3, col=rgb(0, 0, 0, 0.3), border=NA)

}
    
## Kati: black ellipses will show significant population divergence 
viz.2traits <- function (fixedpost, popefpost, Gpost, THpost, traits, prob=0.05, popnames, traits.mat, main = NA, xlab=NULL, ylab=NULL){

    ## popnames: a character vector of short population names for plotting
    require(SIBER) ## Kati: for pointsToEllipsoid function
    ...
    if(is.null(xlab)) xlab = colnames(traits.mat)[traits[1]]
    if(is.null(ylab)) ylab = colnames(traits.mat)[traits[2]]
    M = max(c(sqrt(2 * G[i, i] * diag(TH)), sqrt(2 * G[j, j] * diag(TH)), popef))
    
    ## Kati:
    div.sel <- c()
    myylim <- list(); myxlim <- list()
    for (k in 1:npop) {
        mu = c(ei, ej)
        Sigma = 2 * TH[k, k] * Gthis
        ## Kati: test selection for each population
        Z <- pointsToEllipsoid(matrix(c(ei + popef[k, i], ej + popef[k, j]),ncol=2), Sigma, mu)
        div.sel[k] <- ellipseInOut(Z, p = 1-prob) ## this will determin the color (grey or black) for the ellipse
        myxlim[[k]] <- range(ellipsis(mu, Sigma, 0.5, 1, col=1, plot=F)[,1])
        myylim[[k]] <- range(ellipsis(mu, Sigma, 0.5, 1, col=1, plot=F)[,2])
    }
    plot(ei, ej, pch = 16, cex = 0,
         xlim = range(c(unlist(myxlim), ei + popef[, i])),
         ylim = range(c(unlist(myylim), ej + popef[, j])), xlab = xlab, ylab = ylab, main = main)

    for (k in 1:npop) {
        mu = c(ei, ej)
        Sigma = 2 * TH[k, k] * Gthis
        if(div.sel[k]) ellipsis(mu, Sigma, 0.5, 1, col="grey", plot=TRUE)
        else ellipsis(mu, Sigma, 0.5, 1, col=1, plot=TRUE)
    }
    if(sum(div.sel)==npop) text((ei + popef[, i]), (ej + popef[, j]), popnames, col = "grey", cex=1.5)
    else if(sum(div.sel)==0){
        text((ei + popef[, i])[!div.sel], (ej + popef[, j])[!div.sel], popnames[!div.sel], col = 1, cex=1.5)
    }
    else{
        text((ei + popef[, i])[div.sel], (ej + popef[, j])[div.sel], popnames[div.sel], col = "grey", cex=1.5)
        text((ei + popef[, i])[!div.sel], (ej + popef[, j])[!div.sel], popnames[!div.sel], col = 1, cex=1.5)
    }
    text(ei, ej, "A", cex=2)

    return(popnames[!div.sel]) ## returns names of the populations that are under selection
}
